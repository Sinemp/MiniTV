...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- 所有其他的函数 (toggleSettings, testSiteAvailability 等) 保持不变 ---
    // --- 为了简洁，这里省略了它们，您只需要将它们全部粘贴在这里面 ---
    ...
    function updateSiteStatus(isAvailable) {
        ...
        statusEl.innerHTML = isAvailable ? '<span class="text-green-500">●</span> 可用' : '<span class="text-red-500">●</span> 不可用';
    }
    ...
    async function showDetails(id, vod_name) {
        ...
        try {
            ...
            data.episodes.forEach((episode, index) => {
                ...
                button.onclick = (event) => playVideo(event, episode, vod_name);
    ...
    document.addEventListener('click', function(e) {
        const panel = document.getElementById('settingsPanel');
        // Make sure the button exists before trying to check it
    ...
    // Make functions globally accessible for inline event handlers (onclick)
